...
template<int N,int NMAX> 
void deltaR(int iOffSet, etaphi_t seedeta,etaphi_t seedphi,PFChargedObj pfch[N],axi2_t axis_in[]) { 
  #pragma HLS inline
  #pragma HLS PIPELINE 
  const ap_int<16> eDR2MAX = DR2MAX;
  PFChargedObj dummyc; dummyc.hwPt = 0; dummyc.hwEta = 0; dummyc.hwPhi = 0; dummyc.hwId = 0; dummyc.hwZ0 = 0;
...
template<unsigned int N>
void make_inputs(input_t nn_data[N*8], axi2_t pf[DATA_SIZE]) {
  //#pragma HLS inline
  #pragma HLS PIPELINE
  for (int i = 0; i < N; i++) {
    #pragma HLS PIPELINE II=1
...
void algo_layer2_v3(hls::stream<axi_t> &ch_link_in,hls::stream<axi_t> &ne_link_in, hls::stream<axi_t> &em_link_in, hls::stream<axi_t> &mu_link_in,
  ...
  for(int itau = 0; itau < NTAU; itau++) {
    ...
    dummyc.hwEta = allparts_in[0](31,16);;
    dummyc.hwPhi = allparts_in[0](47,32);;
    dummyc.hwId  = allparts_in[0](63,48);;
